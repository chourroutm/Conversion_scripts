...
def main(keyword):
	intensity_range_origin = np.array([0, 1],dtype=float)
	intensity_range_export = np.array([0, 65535],dtype=float)
	# VALUES TO SET:
	...
	'031_EndoStrokeR21_450mA_u18gap8p5mm_Cu100um_Au20um_mt1_1x_lightning_35ms_3600proj_360_'
	]

	main_dir_list = os.listdir(dirpath_origin)
	for acquisition_dir in main_dir_list:
		if acquisition_dir in dirs_to_skip or keyword.lower() not in acquisition_dir.lower():
		...
		for entry in acquisition_dir_list:
			...
			if entry.endswith(".vol"): # recon volume
				...
				data[data<intensity_range_export[0]] = intensity_range_export[0]
				data[data>intensity_range_export[1]] = intensity_range_export[1]
				data.astype(np.dtype('<u2')).tofile(os.path.join(dirpath_export,acquisition_dir,entry))
				logf = open(os.path.join(dirpath_export,acquisition_dir,"values.txt"), "w")
				logf.write("min = %.5f\nmax = %.5f" % (intensity_range_origin[0], intensity_range_origin[1]))
				logf.close()

parser = argparse.ArgumentParser(
                    prog='convertMultiRawVols',
                    description='convertMultiRawVols',
                    epilog='convertMultiRawVols')
